 ...
 * @description Accepted new ToU
 *
 * @property {boolean} success - Confirms whether or not the new ToU have been successfully accepted
 * /
...
const Celsius = function (config) {
  ...
  const CelsiusInstance = {
    ...
    /**
     * This method checks if given version of ToU has been accepted
     *
     * @memberOf module:celsius-js-sdk/core
     * @function confirmTermsOfUse
     * @param termsOfUseId {string} Id of ToU document
     * @param confirmationDate {Date} Timestamp from which to see if accepted ToU is up to date
     * @param userSecret {string} Represents a secret value used to uniquely identify users. Can be user-token or api-key.
     *
    ...
    /**
     * This method returns statistics for the given userSecret.
     * Timestamp can be optionally passed to configure start date for getting up to date supported countries.
     *
     * @memberOf module:celsius-js-sdk/core
     ...
     */
    getSupportedCountries: function (userSecret, timestamp = null) {
      return httpClient.get(PATHS.SUPPORTED_COUNTRIES, { timestamp }, userSecret)
    },
  }
