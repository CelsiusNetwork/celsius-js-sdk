      ...
      return new Promise(function (resolve, reject) {
        ...
        if (formFields || files) {
          try {
            ...
            return reject(Util.createError(error, {formFields, files}));
          }
        }
        ...
        let req = http.request(httpOptions, function (res) {
          ...
          res.on('end', function() {
            ...
            if (!Util.verifyResponse(bufferedBody, res.headers["x-signature"])) {
              return reject(Util.createError({ message: "Response signature verification failed." }, null));
            }
            ...
            try {
              ...
              reject(Util.createError(error, body));
            }
          })
        });

        req.on('error', function (error) {
          return reject(Util.createError(error, null));
        });
