...
const HttpClient = function (config) {
  ...
  function _getHeaders (config) {
    ...
    if (config.userSecretOptional) {
      delete headers['X-Cel-User-Token']
      delete headers['X-Cel-Api-Key']
      return headers
    }
  ...
  return {
     ...
     */
    get: function (path, queryParams, userSecret, config = {}) {
      const headers = _getHeaders({ userSecret, userSecretOptional: config.userSecretOptional })
